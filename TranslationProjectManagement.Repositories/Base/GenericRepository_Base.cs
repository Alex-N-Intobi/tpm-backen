...
namespace TranslationProjectManagement.Repositories.Base;

public class GenericRepository<TDomain, TDbContext> : IGenericRepository<TDomain>
    where TDomain : DomainBase
    where TDbContext : DbContext
{
    private const string TIMESTAMP = "Timestamp";
    private readonly TDbContext _dbContext;
    public readonly DbSet<TDomain> DbSet;
    ...
    {
        return await GetOneInternalAsync(x => x.Id == id, null, cancellationToken: cancellationToken);
    }

    public virtual async Task<TDomain> GetOneAsync(int id, IEnumerable<Expression<Func<TDomain, object>>> includes, CancellationToken cancellationToken = default)
    {
        includes = includes ?? throw new ArgumentNullException(nameof(includes));

        return await GetOneInternalAsync(x => x.Id == id, includes, cancellationToken);
    }
    ...
    public virtual async Task<TDomain> UpdateOneAsync(TDomain entity, CancellationToken cancellationToken = default)
    {
        DbSet.Update(entity);
    ...
    {
        TDomain entity = await DbSet.SingleOrDefaultAsync(x => x.Id == id, cancellationToken: cancellationToken);
        if (entity == null)
        {
    ...
    protected async Task<T> UpdateOneAsync<T>(T entity, params Expression<Func<T, object>>[] navigations) where T : DomainBase
    {
        entity = entity ?? throw new ArgumentNullException(nameof(entity));
        ...
        dbEntry.Property(TIMESTAMP).OriginalValue = entity.Timestamp;
        ...
        foreach (Expression<Func<T, object>> property in navigations)
        {
            ...
            var dbItemsMap = ((IEnumerable<DomainBase>) dbItemsEntry.CurrentValue).ToDictionary(e => e.Id);

            IClrCollectionAccessor accessor = dbItemsEntry.Metadata.GetCollectionAccessor();
            var items = (IEnumerable<DomainBase>) accessor.GetOrCreate(entity, true);

            foreach (DomainBase item in items)
            {
                if (!dbItemsMap.TryGetValue(item.Id, out DomainBase oldItem))
                {
                    accessor.Add(dbEntity, item, true);
                }
                else
                {
                    _dbContext.Entry(oldItem).Property(TIMESTAMP).OriginalValue = item.Timestamp;
                    _dbContext.Entry(oldItem).CurrentValues.SetValues(item);
                    dbItemsMap.Remove(item.Id);
                }
            }

            foreach (DomainBase oldItem in dbItemsMap.Values)
            {
